...
const statusLabels: Record<Portal['status'], string> = {
  live: 'Live',
  beta: 'Beta',
...
export function StatusBadge({ status }: { status: Portal['status'] }) {
  const styles = status === 'live' ? 'bg-emerald-100 text-emerald-700' : status === 'pending_dns' ? 'bg-amber-100 text-amber-700' : 'bg-slate-100 text-slate-700';

  return <span className={`badge ${styles}`}>{statusLabels[status]}</span>;
}
