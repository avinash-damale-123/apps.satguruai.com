...
export default function Signup() {
  const activeDomains = allowedDomains.filter((domain) => domain.status === 'active').map((domain) => domain.domain);
    ...
    <section className="mx-auto max-w-3xl px-4 py-16">
      <form className="card grid gap-4">
        ...
        <p className="text-sm text-slate-600">
          Approved domains: {activeDomains.join(', ')}. Accounts remain OTP-ready for secure email verification.
        </p>
